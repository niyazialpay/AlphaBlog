...
/*
 * panel/post/media.blade.php karşılığı.
 *
 * Fancybox 5 yerine küçük bir yerel lightbox: tek ekran için CDN'den bir
 * galeri kütüphanesi çekmeye değmez.
 */
...
<template>
  ...
  <div class="flex flex-col gap-3.5 p-[22px]">
    ...
    >
      <div v-for="item in media" :key="item.id" class="p-card group overflow-hidden">
        ...
        <div class="flex items-center gap-1.5 px-2.5 py-2">
          <div class="min-w-0 flex-1">
            <div class="truncate text-[11.5px]">{{ item.name }}</div>
            <div class="text-[10.5px] text-p-ink3">{{ size(item.size) }}</div>
          </div>
          <button class="p-icon-btn" :title="__('media.copy_to_clipboard')" @click="copy(item.url)">
            <i class="fa-solid fa-link"></i>
          </button>
          <button class="p-icon-btn" :title="__('media.show')" @click="preview = item">
