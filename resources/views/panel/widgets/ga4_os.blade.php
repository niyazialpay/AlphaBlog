@php
    $data = $widgetData['ga4']['os'] ?? collect();
    $chartId = 'wc_' . uniqid();
@endphp
<div class="card h-100 mb-0 border-0">
    <div class="card-header py-2 px-3 border-0 bg-transparent">
        <small class="text-muted font-weight-bold">İşletim Sistemleri</small>
    </div>
    <div class="card-body p-2" style="min-height:120px">
        ...
        window.addEventListener('load', function(){
            var labels=[], series=[];
            @foreach($data as $row)
            labels.push('{{ $row['operatingSystem'] }}');
            series.push({{ $row['screenPageViews'] }});
            @endforeach
