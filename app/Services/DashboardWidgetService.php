...
class DashboardWidgetService
{
    ...
    public static function ga4Configured(): bool
    {
        return file_exists(storage_path('app/analytics/service-account-credentials.json'))
            && ! empty(config('analytics.property_id'));
    }
    ...
    public function getDataForWidgets(Collection $widgets): array
    {
        ...
        if ($hasGa4 && ! static::ga4Configured()) {
            $data['status']['ga4'] = 'not_configured';
        } elseif ($hasGa4) {
            ...
            try {
                $data['ga4'] = Cache::remember($cacheKey, now()->addMinutes(15), fn (): array => [
        ...
        $searchConsole = new GoogleSearchConsoleService;

        if ($hasGsc && ! $searchConsole->isConfigured()) {
            $data['status']['gsc'] = 'not_configured';
        } elseif ($hasGsc) {
            $end = Carbon::now();
            $start = Carbon::now()->subDays(7);
            ...
            try {
                $data['gsc'] = Cache::remember($cacheKey, now()->addMinutes(15), fn (): array => [
                    'performance' => $searchConsole->getPerformance($start, $end),
                    'keywords' => $searchConsole->getKeywords($start, $end, limit: 10),
                    'trend' => $searchConsole->getClicksTrend($start, $end),
                ]);
                $data['status']['gsc'] = 'ok';
            } catch (Throwable $e) {
                Log::error('Dashboard Search Console widget verisi alinamadi', ['exception' => $e]);
                $data['gsc'] = ['performance' => [], 'keywords' => [], 'trend' => []];
                $data['status']['gsc'] = 'error';
            }
        }
