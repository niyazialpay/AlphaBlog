...
class FirewallMiddleware
{
    ...
    protected function blockRequest(string $reason, Request $request, Firewall $firewall, array $ipList): void
    {
        ...
        // Merge all IPs with whitelistedBotIPS to bypass if it's in a bot IP list
     ...
     * or /128 for IPv6 before passing to IpUtils::checkIp.
     *
     * @param string $clientIp
     * @param string[] $ips
     * @return bool
    ...
    protected function checkIpInList(string $clientIp, array $ips): bool
    {
        foreach ($ips as $ipItem) {
            ...
            // If not a CIDR, determine whether it's IPv4 or IPv6 and append the correct mask
            if (!str_contains($ipItem, '/')) {
                if (filter_var($ipItem, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6)) {
                    $ipItem .= '/128';
                } else {
                    $ipItem .= '/32';
                }
            ...
            if (IpUtils::checkIp($clientIp, $ipItem)) {
                return true;
            }
        }
